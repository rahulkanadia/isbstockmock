...
async function main() {
  ...
  // Clean wipe in order of dependency
  ...
  for (const u of legacyUsers) {
    const market = symbolMap.get(u.symbol);
    const currentPrice = market?.janClose ?? u.price;
    const prevMonthClose = market?.janClose ?? u.price;

    // Pre-calculate returns so dashboard is hydrated immediately
    const seasonReturn = ((currentPrice - u.price) / u.price) * 100;
  ...
  console.log("✅ Seed complete. 100+ Users synchronized with January benchmarks.");
}
