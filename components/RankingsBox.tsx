...
export default function RankingsBox({ 
  ...
  const UserCard = ({ user, rank, type, icon: Icon }: any) => {
    ...
    const isPending = user.pick?.symbol === "PENDING";
      ...
      >
         <div className="flex items-center gap-2 overflow-hidden">
            ...
            <div className="flex flex-col truncate">
               ...
               <span className={cn("text-[8px] font-mono opacity-60 truncate", isMe ? "text-gray-300" : "text-gray-400")}>
                 {isPending ? "PENDING" : user.pick?.symbol?.split('.')[0]}
               </span>
            </div>
         </div>
         <div className={cn("text-[10px] font-mono font-bold", returnVal >= 0 ? "text-success" : "text-danger", isPending && "!text-gray-400")}>
                ...
                <div className="flex-1 bg-ink rounded-lg p-2 flex flex-col justify-between relative overflow-hidden group cursor-pointer" onClick={() => last && activeUser?.username !== last.username && onSetNemesis(last)}>
                   ...
                   {last && (
                     <>
                       ...
                       <div className="relative z-10 text-[8px] font-mono text-gray-500 text-center">{last.pick?.symbol === "PENDING" ? "PENDING" : last.pick?.symbol?.split('.')[0]}</div>
                     </>
                   )}
                </div>
             </div>
                ...
                >
                   <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className={cn("text-[9px] font-mono mt-0.5", isMe ? "text-gray-400" : "text-gray-400")}>{isPending ? "PENDING" : u.pick.symbol.split('.')[0]}</span>
                      </div>
                   </div>
                   <div className="flex items-center gap-2">
                      <span className={cn("font-mono font-bold text-xs", returnVal >= 0 ? "text-success" : "text-danger", isPending && "!text-gray-400")}>{isPending ? "0.0%" : `${returnVal > 0 ? "+" : ""}${returnVal.toFixed(2)}%`}</span>
                      {!isMe && (
